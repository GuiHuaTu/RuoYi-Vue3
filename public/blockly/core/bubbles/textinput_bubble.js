...
export class TextInputBubble extends Bubble {
    ...
    bindTextAreaEvents(textArea) {
        ...
        browserEvents.conditionalBind(textArea, 'focus', this, this.onStartEdit, true);
        browserEvents.conditionalBind(textArea, 'change', this, this.onTextChange);
    }
    /** Creates the resize handler elements and binds events to them. */
    createResizeHandle(container) {
        const resizeGroup = dom.createSvgElement(Svg.G, {
            'class': this.workspace.RTL ? 'blocklyResizeSW' : 'blocklyResizeSE',
        }, container);
        const size = 2 * Bubble.BORDER_WIDTH;
        dom.createSvgElement(Svg.POLYGON, { 'points': `0,${size} ${size},${size} ${size},0` }, resizeGroup);
        ...
        browserEvents.conditionalBind(resizeGroup, 'pointerdown', this, this.onResizePointerDown);
        return resizeGroup;
    }
    /**
    ...
    setSize(size, relayout = false) {
        ...
        if (this.workspace.RTL) {
            this.resizeGroup.setAttribute('transform', `translate(${Bubble.DOUBLE_BORDER}, ${heightMinusBorder}) scale(-1 1)`);
        }
        else {
            this.resizeGroup.setAttribute('transform', `translate(${widthMinusBorder}, ${heightMinusBorder})`);
        }
    ...
    onResizePointerDown(e) {
        ...
        this.workspace.startDrag(e, new Coordinate(this.workspace.RTL ? -this.getSize().width : this.getSize().width, this.getSize().height));
