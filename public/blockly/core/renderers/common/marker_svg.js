...
export class MarkerSvg {
    ...
    showAtLocation_(curNode) {
        const curNodeAsConnection = curNode.getLocation();
     ...
     */
    showWithBlockPrevOutput(curNode) {
    ...
    showWithField_(curNode) {
        const field = curNode.getLocation();
    ...
    positionBlock_(width, markerOffset, markerHeight) {
        ...
        if (!this.markerBlock_) {
            throw new Error('createDom should be called before positioning the marker');
        }
    ...
    positionInput_(connection) {
        ...
        this.markerInput_.setAttribute('transform', 'translate(' +
            x +
            ',' +
            y +
            ')' +
    ...
    positionOutput_(width, height, connectionShape) {
        if (!this.markerBlock_) {
            throw new Error('createDom should be called before positioning the output');
        }
    ...
    positionPrevious_(width, markerOffset, markerHeight, connectionShape) {
        if (!this.markerBlock_) {
            throw new Error('createDom should be called before positioning the previous connection marker');
        }
        const markerPath = svgPaths.moveBy(-markerOffset, markerHeight) +
            svgPaths.lineOnAxis('V', -markerOffset) +
     ...
     */
    fireMarkerEvent(oldNode, curNode) {
        const curBlock = curNode.getSourceBlock();
        const event = new (eventUtils.get(eventUtils.MARKER_MOVE))(curBlock, this.isCursor(), oldNode, curNode);
    ...
    createDomInternal_() {
        ...
        if (this.isCursor()) {
            ...
            dom.createSvgElement(Svg.ANIMATE, { ...blinkProperties, attributeName: 'stroke' }, this.markerBlock_);
        }
    ...
    applyColour_(_curNode) {
        if (!this.markerSvgLine_ ||
            ...
            !this.markerInput_ ||
            !this.markerBlock_) {
            throw new Error('createDom should be called before applying color to the markerj');
        }
