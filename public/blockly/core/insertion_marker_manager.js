...
export class InsertionMarkerManager {
    ...
    constructor(block) {
        ...
        if (this.lastOnStack) {
            this.lastMarker = this.createMarkerBlock(this.lastOnStack.getSourceBlock());
        }
    }
    /**
    ...
    update(dxy, dragTarget) {
        ...
        const shouldUpdate = this.wouldDeleteBlock || this.shouldUpdatePreviews(newCandidate, dxy);
        if (shouldUpdate) {
     ...
     */
    shouldUpdatePreviews(newCandidate, dxy) {
        ...
        return (newCandidate.radius < curDistance - config.currentConnectionPreference);
    }
    /**
     ...
     *     best candidate.
     */
    maybeShowPreview(newCandidate) {
        if (this.wouldDeleteBlock)
            return; // Nope, don't add a marker.
        if (!newCandidate)
     ...
     *     immediately.
     */
    showPreview(activeCandidate) {
        const renderer = this.workspace.getRenderer();
        const method = renderer.getConnectionPreviewMethod(activeCandidate.closest, activeCandidate.local, this.topBlock);
        switch (method) {
    ...
    maybeHidePreview(newCandidate) {
        ...
        if (!newCandidate) {
            ...
            if (this.activeCandidate) {
                const closestChanged = this.activeCandidate.closest !== newCandidate.closest;
     ...
     *     immediately.
     */
    showInsertionMarker(activeCandidate) {
        ...
        if (!insertionMarker) {
            throw new Error('Cannot show the insertion marker because there is no insertion ' +
                'marker block');
        }
        let imConn;
        try {
            imConn = insertionMarker.getMatchingConnection(local.getSourceBlock(), local);
        }
        catch (e) {
        ...
        if (imConn === this.markerConnection) {
            throw new Error("Made it to showInsertionMarker_ even though the marker isn't " +
                'changing');
        }
    ...
    hideInsertionMarker() {
        ...
        if (markerConn.targetConnection) {
            throw Error('markerConnection still connected at the end of ' +
     ...
     *     immediately.
     */
    showInsertionInputOutline(activeCandidate) {
    ...
    hideInsertionInputOutline() {
        if (!this.highlightedBlock)
            return;
        if (!this.activeCandidate) {
            throw new Error('Cannot hide the insertion marker outline because ' +
                'there is no active candidate');
        }
        this.highlightedBlock.highlightShapeForInput(this.activeCandidate.closest, false);
        this.highlightedBlock = null;
    }
    /**
     ...
     *     immediately.
     */
    showReplacementFade(activeCandidate) {
        this.fadedBlock = activeCandidate.closest.targetBlock();
        if (!this.fadedBlock) {
            throw new Error('Cannot show the replacement fade because the ' +
...
(function (InsertionMarkerManager) {
    ...
    (function (PREVIEW_TYPE) {
        PREVIEW_TYPE[PREVIEW_TYPE["INSERTION_MARKER"] = 0] = "INSERTION_MARKER";
        PREVIEW_TYPE[PREVIEW_TYPE["INPUT_OUTLINE"] = 1] = "INPUT_OUTLINE";
        PREVIEW_TYPE[PREVIEW_TYPE["REPLACEMENT_FADE"] = 2] = "REPLACEMENT_FADE";
    })(PREVIEW_TYPE = InsertionMarkerManager.PREVIEW_TYPE || (InsertionMarkerManager.PREVIEW_TYPE = {}));
})(InsertionMarkerManager || (InsertionMarkerManager = {}));
export const PreviewType = InsertionMarkerManager.PREVIEW_TYPE;
//# sourceMappingURL=insertion_marker_manager.js.map