...
function generateUniqueNameInternal(workspace) {
    return generateUniqueNameFromOptions(VAR_LETTER_OPTIONS.charAt(0), workspace.getAllVariableNames());
}
/**
...
export function renameVariable(workspace, variable, opt_callback) {
    // This function needs to be named so it can be called recursively.
    function promptAndCheckWithAlert(defaultName) {
        const promptText = Msg['RENAME_VARIABLE_TITLE'].replace('%1', variable.name);
        promptName(promptText, defaultName, function (newName) {
            ...
            const procedure = nameUsedWithConflictingParam(variable.name, newName, workspace);
