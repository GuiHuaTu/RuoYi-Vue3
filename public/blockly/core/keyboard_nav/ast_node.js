...
export class ASTNode {
    ...
    findNextForField() {
        ...
        if (!block) {
            throw new Error('The current AST location is not associated with a block');
        }
    ...
    navigateBetweenStacks(forward) {
        ...
        if (curLocation.getSourceBlock) {
            const block = curLocation.getSourceBlock();
            if (block) {
                curLocation = block;
            }
        }
        // TODO(#6097): Use instanceof checks to exit early for values of
        // curLocation that don't make sense.
        const curLocationAsBlock = curLocation;
    ...
    getOutAstNodeForBlock(block) {
        ...
        if (topConnection &&
            ...
            // AnyDuringMigration because:  Argument of type 'Input | null' is not
            // assignable to parameter of type 'Input'.
            return ASTNode.createInputNode(topConnection.targetConnection.getParentInput());
        }
        else {
    ...
    next() {
        switch (this.type) {
            ...
            case ASTNode.types.OUTPUT: {
                const connection = this.location;
    ...
    out() {
        switch (this.type) {
            case ASTNode.types.STACK: {
                ...
                const wsCoordinate = new Coordinate(blockPos.x, blockPos.y + ASTNode.DEFAULT_OFFSET_Y);
...
(function (ASTNode) {
    ...
    })(types = ASTNode.types || (ASTNode.types = {}));
})(ASTNode || (ASTNode = {}));
