...
export class Block {
    ...
    setInputsInline(newBoolean) {
        if (this.inputsInline !== newBoolean) {
            eventUtils.fire(new (eventUtils.get(eventUtils.BLOCK_CHANGE))(this, 'inline', null, this.inputsInline, newBoolean));
    ...
    setEnabled(enabled) {
        if (this.isEnabled() !== enabled) {
            ...
            eventUtils.fire(new (eventUtils.get(eventUtils.BLOCK_CHANGE))(this, 'disabled', null, oldValue, !enabled));
        }
    }
    /**
     ...
     * @param warningPrefix Warning prefix string identifying block.
     */
    interpolate_(message, args, implicitAlign, warningPrefix) {
     ...
     */
    interpolateArguments_(tokens, args, implicitAlign) {
        const elements = [];
        for (let i = 0; i < tokens.length; i++) {
        ...
        if (length &&
            !this.isInputKeyword_(elements[length - 1]['type'])) {
     ...
     */
    getCommentText() {
        const comment = this.getIcon(CommentIcon.TYPE);
    ...
    setCommentText(text) {
        ...
        if (oldText === text)
            return;
        if (text !== null) {
            let comment = this.getIcon(CommentIcon.TYPE);
            if (!comment) {
        ...
        eventUtils.fire(new (eventUtils.get(eventUtils.BLOCK_CHANGE))(this, 'comment', null, oldText, text));
    }
    /**
