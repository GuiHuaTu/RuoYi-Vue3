 ...
 * @param rtl True if RTL, false if LTR.
 */
function position_(menu, e, rtl) {
    // Record windowSize and scrollOffset before adding menu.
    const viewportBBox = svgMath.getViewportBBox();
    const mouseEvent = e;
    // This one is just a point, but we'll pretend that it's a rect so we can use
    // some helper functions.
    const anchorBBox = new Rect(mouseEvent.clientY + viewportBBox.top, mouseEvent.clientY + viewportBBox.top, mouseEvent.clientX + viewportBBox.left, mouseEvent.clientX + viewportBBox.left);
    createWidget_(menu);
...
export function workspaceCommentOption(ws, e) {
    ...
    function addWsComment() {
        const comment = new WorkspaceCommentSvg(ws, Msg['WORKSPACE_COMMENT_DEFAULT_TEXT'], WorkspaceCommentSvg.DEFAULT_SIZE, WorkspaceCommentSvg.DEFAULT_SIZE);
        ...
        const mouseEvent = e;
        const clientOffsetPixels = new Coordinate(mouseEvent.clientX - boundingRect.left, mouseEvent.clientY - boundingRect.top);
