...
export class Trashcan extends DeleteArea {
    ...
    init() {
        if (this.workspace.options.maxTrashcanContents > 0) {
            dom.insertAfter(this.flyout.createDom(Svg.SVG), this.workspace.getParentSvg());
    ...
    fireUiEvent(trashcanOpen) {
        const uiEvent = new (eventUtils.get(eventUtils.TRASHCAN_OPEN))(trashcanOpen, this.workspace.id);
    ...
    onDelete(event) {
        ...
        if (event.type === eventUtils.BLOCK_DELETE && !deleteEvent.wasShadow) {
            ...
            const cleanedJson = JSON.stringify(this.cleanBlockJson(deleteEvent.oldJson));
