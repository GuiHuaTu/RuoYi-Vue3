 ...
 */
function disposeUiStep(clone, rect, rtl, start, workspaceScale) {
    ...
    if (percent > 1) {
        ...
        const x = rect.x + (((rtl ? -1 : 1) * rect.width * workspaceScale) / 2) * percent;
 ...
 * @param start Date of animation's start.
 */
function disconnectUiStep(block, magnitude, start) {
    ...
    let skew = '';
    if (percent <= 1) {
        const val = Math.round(Math.sin(percent * Math.PI * WIGGLES) * (1 - percent) * magnitude);
        skew = `skewX(${val})`;
