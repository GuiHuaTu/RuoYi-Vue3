...
export class WorkspaceSvg extends Workspace {
    ...
    constructor(options) {
        ...
        if (Variables && Variables.flyoutCategory) {
            this.registerToolboxCategoryCallback(Variables.CATEGORY_NAME, Variables.flyoutCategory);
        }
        if (VariablesDynamic && VariablesDynamic.flyoutCategory) {
            this.registerToolboxCategoryCallback(VariablesDynamic.CATEGORY_NAME, VariablesDynamic.flyoutCategory);
        }
        if (Procedures && Procedures.flyoutCategory) {
            this.registerToolboxCategoryCallback(Procedures.CATEGORY_NAME, Procedures.flyoutCategory);
    ...
    refreshTheme() {
        ...
        const event = new (eventUtils.get(eventUtils.THEME_CHANGE))(this.getTheme().name, this.id);
    ...
    createDom(opt_backgroundClass, injectionDiv) {
        ...
        if (!this.isFlyout) {
            browserEvents.conditionalBind(this.svgGroup_, 'pointerdown', this, this.onMouseDown_, false);
    ...
    maybeFireViewportChangeEvent() {
        ...
        const event = new (eventUtils.get(eventUtils.VIEWPORT_CHANGE))(top, left, scale, this.id, this.oldScale);
        this.oldScale = scale;
        this.oldTop = top;
        this.oldLeft = left;
        eventUtils.fire(event);
     ...
     */
    paste(state) {
        deprecation.warn('Blockly.WorkspaceSvg.prototype.paste', 'v10', 'v11', 'Blockly.clipboard.paste');
        ...
        try {
            ...
            if (state['type']) {
                pastedThing = this.pasteBlock_(null, state);
            }
            else {
                const xmlBlock = state;
    ...
    pasteBlock_(xmlBlock, jsonBlock) {
        ...
        try {
            ...
            let blockY = 0;
            if (xmlBlock) {
                block = Xml.domToBlockInternal(xmlBlock, this);
                ...
                do {
                    ...
                    if (!collide) {
                        ...
                        for (let i = 0, connection; (connection = connections[i]); i++) {
                            const neighbour = connection.closest(config.snapRadius, 
    ...
    startDrag(e, xy) {
        ...
        const point = browserEvents.mouseToSvg(e, this.getParentSvg(), this.getInverseScreenCTM());
    ...
    showContextMenu(e) {
        ...
        const menuOptions = ContextMenuRegistry.registry.getContextMenuOptions(ContextMenuRegistry.ScopeType.WORKSPACE, { workspace: this });
