...
export class Flyout extends DeleteArea {
    ...
    createFlyoutBlock(blockInfo) {
        ...
        if (blockInfo['blockxml']) {
            ...
            if (!block) {
                ...
                block = blocks.appendInternal(blockInfo, this.workspace_);
            }
        }
        if (!block.isEnabled()) {
    ...
    addBlockListeners_(root, block, rect) {
        ...
        this.listeners.push(browserEvents.bind(root, 'pointerenter', block, () => {
            if (!this.targetWorkspace.isDragging()) {
                block.addSelect();
            }
        }));
        this.listeners.push(browserEvents.bind(root, 'pointerleave', block, () => {
            if (!this.targetWorkspace.isDragging()) {
                block.removeSelect();
            }
        }));
        this.listeners.push(browserEvents.bind(rect, 'pointerenter', block, () => {
            if (!this.targetWorkspace.isDragging()) {
                block.addSelect();
            }
        }));
        this.listeners.push(browserEvents.bind(rect, 'pointerleave', block, () => {
            if (!this.targetWorkspace.isDragging()) {
                block.removeSelect();
            }
        }));
