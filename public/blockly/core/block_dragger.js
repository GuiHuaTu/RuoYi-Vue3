...
export class BlockDragger {
    ...
    fireDragStartEvent_() {
        const event = new (eventUtils.get(eventUtils.BLOCK_DRAG))(this.draggingBlock_, true, this.draggingBlock_.getDescendants(false));
    ...
    endDrag(e, currentDragDeltaXY) {
        ...
        if (!deleted) {
            ...
            if (delta) {
                ...
                // bounds.
                bumpObjects.bumpIntoBounds(this.draggingBlock_.workspace, this.workspace_.getMetricsManager().getScrollMetrics(true), this.draggingBlock_);
            }
        }
    ...
    fireDragEndEvent_() {
        const event = new (eventUtils.get(eventUtils.BLOCK_DRAG))(this.draggingBlock_, false, this.draggingBlock_.getDescendants(false));
        eventUtils.fire(event);
    }
    /**
    ...
    updateToolboxStyle_(isEnd) {
        ...
        if (toolbox) {
            ...
            // type 'IToolbox'.
            if (isEnd &&
                typeof toolbox.removeStyle === 'function') {
                // AnyDuringMigration because:  Property 'removeStyle' does not exist on
                // type 'IToolbox'.
                ...
                typeof toolbox.addStyle === 'function') {
                // AnyDuringMigration because:  Property 'addStyle' does not exist on
                // type 'IToolbox'.
    ...
    pixelsToWorkspaceUnits_(pixelCoord) {
        const result = new Coordinate(pixelCoord.x / this.workspace_.scale, pixelCoord.y / this.workspace_.scale);
 ...
 */
function initIconData(block, blockOrigin) {
    ...
    for (const child of block.getChildren(false)) {
        dragIconData.push(...initIconData(child, Coordinate.sum(blockOrigin, child.relativeCoords)));
    }
...
//# sourceMappingURL=block_dragger.js.map