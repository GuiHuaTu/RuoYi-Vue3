...
export class WorkspaceCommentSvg extends WorkspaceComment {
    ...
    initSvg(opt_noSelect) {
        ...
        if (!this.workspace.options.readOnly && !this.eventsInit) {
            browserEvents.conditionalBind(this.svgRectTarget, 'pointerdown', this, this.pathMouseDown);
            browserEvents.conditionalBind(this.svgHandleTarget, 'pointerdown', this, this.pathMouseDown);
        }
    ...
    toXmlWithXY(opt_noId) {
        ...
        const xy = this.getRelativeToSurfaceXY();
        element.setAttribute('x', String(Math.round(this.workspace.RTL ? width - xy.x : xy.x)));
    ...
    render() {
        ...
        this.rendered = true;
        if (this.resizeGroup) {
            browserEvents.conditionalBind(this.resizeGroup, 'pointerdown', this, this.resizeMouseDown);
        }
        if (this.isDeletable()) {
            browserEvents.conditionalBind(this.deleteGroup, 'pointerdown', this, this.deleteMouseDown);
            browserEvents.conditionalBind(this.deleteGroup, 'pointerout', this, this.deleteMouseOut);
            browserEvents.conditionalBind(this.deleteGroup, 'pointerup', this, this.deleteMouseUp);
        }
    }
    /**
    ...
    addResizeDom() {
        this.resizeGroup = dom.createSvgElement(Svg.G, { 'class': this.RTL ? 'blocklyResizeSW' : 'blocklyResizeSE' }, this.svgGroup);
        dom.createSvgElement(Svg.POLYGON, {
    ...
    addDeleteDom() {
        this.deleteGroup = dom.createSvgElement(Svg.G, { 'class': 'blocklyCommentDeleteIcon' }, this.svgGroup);
        this.deleteIconBorder = dom.createSvgElement(Svg.CIRCLE, { 'class': 'blocklyDeleteIconShape', 'r': '7', 'cx': '7.5', 'cy': '7.5' }, this.deleteGroup);
        // x icon.
        dom.createSvgElement(Svg.LINE, {
            'x1': '5',
            'y1': '10',
    ...
    resizeMouseDown(e) {
        ...
        this.workspace.startDrag(e, new Coordinate(this.workspace.RTL ? -this.width_ : this.width_, this.height_));
        this.onMouseUpWrapper = browserEvents.conditionalBind(document, 'pointerup', this, this.resizeMouseUp);
        this.onMouseMoveWrapper = browserEvents.conditionalBind(document, 'pointermove', this, this.resizeMouseMove);
    ...
    setSize(width, height) {
        ...
        this.svgHandleTarget?.setAttribute('height', String(WorkspaceCommentSvg.TOP_OFFSET));
        if (this.RTL) {
        ...
        if (this.resizeGroup) {
            if (this.RTL) {
                ...
                this.resizeGroup.setAttribute('transform', 'translate(' +
                    (-width + RESIZE_SIZE) +
                    ',' +
                    (height - RESIZE_SIZE) +
                    ') scale(-1 1)');
                this.deleteGroup?.setAttribute('transform', 'translate(' +
                    (-width + RESIZE_SIZE) +
                    ',' +
                    -RESIZE_SIZE +
                    ') scale(-1 1)');
                ...
                this.resizeGroup.setAttribute('transform', 'translate(' +
                    ...
                    (height - RESIZE_SIZE) +
                    ')');
                this.deleteGroup?.setAttribute('transform', 'translate(' + (width - RESIZE_SIZE) + ',' + -RESIZE_SIZE + ')');
            }
        }
    ...
    blurFocus() {
        ...
        setTimeout(() => {
            ...
            if (this.svgHandleTarget) {
                dom.removeClass(this.svgHandleTarget, 'blocklyCommentHandleTargetFocused');
            }
        }, 0);
    }
    /**
    ...
    static fromXmlRendered(xmlComment, workspace, opt_wsWidth) {
        ...
        try {
            ...
            comment = new WorkspaceCommentSvg(workspace, info.content, info.h, info.w, info.id);
...
`);
//# sourceMappingURL=workspace_comment_svg.js.map