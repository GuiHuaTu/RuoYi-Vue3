...
export class BubbleDragger {
    ...
    shouldDelete_(dragTarget) {
        if (dragTarget) {
            ...
            if (isDeleteArea) {
                return dragTarget.wouldDelete(this.bubble, false);
            }
        }
        return false;
    }
    ...
    endBubbleDrag(e, currentDragDeltaXY) {
        ...
        const preventMove = this.dragTarget_ && this.dragTarget_.shouldPreventMove(this.bubble);
        let newLoc;
        if (preventMove) {
    ...
    fireMoveEvent_() {
        if (this.bubble instanceof WorkspaceCommentSvg) {
            const event = new (eventUtils.get(eventUtils.COMMENT_MOVE))(this.bubble);
...
//# sourceMappingURL=bubble_dragger.js.map